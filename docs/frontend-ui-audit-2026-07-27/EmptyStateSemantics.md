...
| Line | Element | Verdict | Reason | Suggested change |
|---|---|---|---|---|
| `src/features/KanbanBoard/components/TaskImpactLine/index.tsx:72` | Missing impact value | fix | `CircleSlash` plus `N/A` visually implied failure for ordinary missing optional impact data. | Render a quiet dash while retaining the accessible name `No code changes`. |
| `src/components/ContextListDropdown/index.tsx:194` | Empty context list | fix | The same `AlertCircle` used for load failures was also used for a normal empty result. | Keep the error icon only for the error branch; render the normal empty message without an alert icon. |
...
- Migrated sites: Kanban impact, testing framework label, image diff metadata, CLI installation source, CLI API/protocol lists, and the internal flow-awareness summary.
