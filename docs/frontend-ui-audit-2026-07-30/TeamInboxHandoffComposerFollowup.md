...
| `SessionHandoffComposer.tsx:107-110,251-255` | Client-side validation feedback | fix | The primary action becomes disabled for an invalid destination or recipient, but only the later server error is rendered. A roster refresh can therefore leave the user with a disabled action and no explanation. | Render the localized `validationError` beside the affected field or in the existing alert region, and cover stale-destination / stale-recipient states. |
