...
| --------------------------------- | ------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Real Codex first open             | PASS    | The rendered Tauri app indexes the isolated external root through the production rescan command, resolves the imported Codex session through the production bounded-replay adapter, and opens the chat session. | 26 events were hydrated; first-open Physical Footprint did not exceed the sampled 529.4 MiB startup baseline, against the 400 MiB growth limit.                 |
...
| #435 native attribution           | PASS    | Every process row returned by `get_app_memory_snapshot_v1` is compared with `/usr/bin/vmmap -summary`.                                                                                                          | Native total 529.4 MiB versus vmmap 490.6 MiB; 38.8 MiB difference is within `max(10%, 50 MiB)`.                                                                |
...
- `E2E_ISOLATED_RUN=1 E2E_ORGII_HOME=<temp> ORGII_EXTERNAL_HISTORY_HOME=<issue272-only-temp> E2E_ISSUE_443_REAL_CODEX_SESSION_ID=codexapp-rollout-2026-07-12T01-13-48-019f522b-c7de-7c50-885f-ab6790d68469 E2E_CHAT_RENDERING_SCENARIOS=issue-443-real-codex pnpm test -- --spec './specs/core/external-replay-ui.spec.mjs'`: PASS on the final pre-commit tree, one real rendered scenario in 1m 45.6s (1m 55s including runner startup/cleanup).
