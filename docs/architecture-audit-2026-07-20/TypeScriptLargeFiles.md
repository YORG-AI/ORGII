...
|    8 | `src/modules/WorkStation/Browser/Panels/BrowserMainPane/content/TokenManagerContent/index.tsx`                                     |   82 | token-management coordinator     | completed             | Reduced from 451 LOC; pure filtering/color derivation, section navigation, single/consolidated views, and TokenCard now have explicit owners. Removed the never-used `onTokensChange` prop and added disclosure semantics.                        |
...
`model → useTokenSectionNavigation → TokenCard + single/consolidated views → TokenManager coordinator → Browser hosts`

- `model.ts` owns case-insensitive search, category lookup, counts, immutable collapse toggling, all-collapsed detection, and runtime color-style normalization with pure tests.
- `useTokenSectionNavigation.ts` exclusively owns section refs, anchor scrolling, scroll-position tracking, and collapse/expand state.
- The coordinator owns `useGlobalTokens`, search state, consolidated-vs-single routing, and one-way data flow into leaf views.
- The never-used private `onTokensChange` prop and unused `tokens` hook result were removed after a full caller sweep; the original default/named component exports remain unchanged.
- Section disclosure now provides `aria-expanded` and `aria-controls` without changing layout or interaction.
