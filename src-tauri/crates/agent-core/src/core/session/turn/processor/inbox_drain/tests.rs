//! Test suite for the per-agent inbox drain hook.
//!
//! Moved out of `mod.rs` verbatim (mechanical `mod tests { .. }` →
//! `mod tests;` extraction, no logic changes) to keep the parent module
//! focused on its public re-export surface. See `mod.rs` for the
//! `drain_and_render[_deferred]` entry points these tests exercise, and
//! `drain.rs` / `render.rs` / `guard.rs` / `hooks.rs` for the
