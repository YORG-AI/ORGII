...
#[derive(Debug, Clone)]
pub(in crate::sources::imported_history::replay) struct SidecarProbe {
    files: Vec<ProbedFile>,
    pub(in crate::sources::imported_history::replay) signature: String,
...
pub(in crate::sources::imported_history::replay) struct QoderSidecarCursor {
    #[serde(default)]
    version: u32,
    #[serde(default)]
    signature: String,
...
#[derive(Debug)]
pub(in crate::sources::imported_history::replay) struct SidecarSyncOutcome {
...
pub(in crate::sources::imported_history::replay) fn probe(
    source_session_id: &str,
) -> Result<SidecarProbe, String> {
    let task_dir_name = source_session_id
        .split_once('/')
        .map_or(source_session_id, |(_, task)| task);
...
pub(in crate::sources::imported_history::replay) fn cursor_signature(
    cursor_json: &str,
) -> Option<String> {
    #[derive(Deserialize)]
    struct Wrapper {
