...
use super::signals::{SessionSignals, SessionSignalsAccumulator, SIGNALS_VERSION};
use crate::sources::imported_history::{
    replay::{ReplayCursor, ReplayLimits},
    router::scan_activity_chunks_for_session,
};
...
pub fn recompute_session_signals(
    ...
    visit_session_chunks(conn, session_id, |chunk| {
        saw_chunk = true;
    ...
    let s = accumulator.finish();
    upsert(conn, &s)?;
    Ok(Some(s))
}

/// Visit one imported transcript without retaining a session-sized chunk
/// vector. The replay cursor pins one generation/revision for the whole scan;
/// every page remains subject to the normal event and IPC byte limits.
fn visit_session_chunks(
    conn: &mut Connection,
    session_id: &str,
    mut visit: impl FnMut(&ActivityChunk),
) -> Result<(), String> {
    ...
    loop {
        let Some(scan) =
            scan_activity_chunks_for_session(conn, session_id, cursor.as_ref(), limits)?
        else {
            return Ok(());
        };
        ...
        let previous_sequence = cursor
            .as_ref()
            .map_or(-1, |previous| previous.through_sequence);
        if scan.chunks.is_empty() || scan.cursor.through_sequence <= previous_sequence {
            return Err(format!(
                "Builder Profile replay scan made no progress for {session_id} after sequence {previous_sequence}"
            ));
        }
        cursor = Some(scan.cursor);
    }
}
...
pub fn backfill_session_signals(conn: &mut Connection, limit: usize) -> Result<usize, String> {
...
mod tests {
    ...
    fn backfill_is_a_no_op_without_the_imported_cache() {
        let mut conn = db();
        assert_eq!(
            backfill_session_signals(&mut conn, 10).expect("backfill"),
            0
        );
    }
    ...
    fn unreadable_sessions_do_not_stall_the_backfill() {
        ...
        let first = backfill_session_signals(&mut conn, 10).expect("first pass");
        assert_eq!(
            first, 3,
            "unreadable sessions must still count as processed, or the drain loop stops with a stranded backlog"
        );
        let second = backfill_session_signals(&mut conn, 10).expect("second pass");
        assert_eq!(
            second, 0,
...
#[cfg(test)]
struct RealHistoryCopy {
    // Field order matters: close SQLite before TempDir removes the files.
    conn: Connection,
    _dir: tempfile::TempDir,
}

#[cfg(test)]
fn open_real_history_copy(path: &str) -> Result<RealHistoryCopy, String> {
    let dir = tempfile::tempdir().map_err(|err| format!("create history snapshot: {err}"))?;
    let copy_path = dir.path().join("sessions.db");
    std::fs::copy(path, &copy_path)
        .map_err(|err| format!("copy history database into test snapshot: {err}"))?;
    for suffix in ["-wal", "-shm"] {
        let source = std::path::PathBuf::from(format!("{path}{suffix}"));
        if source.exists() {
            std::fs::copy(&source, dir.path().join(format!("sessions.db{suffix}")))
                .map_err(|err| format!("copy history database{suffix}: {err}"))?;
        }
    }
    let conn =
        Connection::open(&copy_path).map_err(|err| format!("open history snapshot: {err}"))?;
    Ok(RealHistoryCopy { conn, _dir: dir })
}
...
/// write to a store the running app owns. The test copies the database (and
/// any live WAL) into a temporary directory before replay indexing. Run with
/// `cargo test -p orgtrack_core real_local_history -- --ignored --nocapture`.
#[cfg(test)]
mod real_data {
    use super::*;
    use crate::profile::{self, signals};
    ...
    fn real_local_history_produces_a_profile() {
        ...
        let mut snapshot = open_real_history_copy(&path).expect("open history snapshot");
        ...
        for (id, source) in &ids {
            let mut accumulator = SessionSignalsAccumulator::new(id, source);
            let mut chunk_count = 0usize;
            if visit_session_chunks(&mut snapshot.conn, id, |chunk| {
                chunk_count += 1;
                accumulator.push(chunk);
            })
            .is_ok()
...
mod taxonomy {
    ...
    fn report_real_tool_names() {
        ...
        for id in &ids {
            let _ = visit_session_chunks(&mut snapshot.conn, id, |c| {
                if c.action_type == "tool_call" {
