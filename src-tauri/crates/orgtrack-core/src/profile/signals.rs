...
/// Incremental profile projection for one replay scan.
///
/// External histories are read through bounded replay pages. Keeping the
/// folding state separate from page storage prevents a large transcript from
/// becoming a session-sized `Vec<ActivityChunk>` just to compute aggregates.
pub struct SessionSignalsAccumulator {
    ...
    questions: i64,
    redirects: i64,
    postedit_evidence: i64,
    postedit_moveon: i64,
    postedit_paste: i64,
    pending_postedit: bool,
    first_brief_seen: bool,
    lines_added: i64,
    lines_removed: i64,
    planned_first: bool,
    edit_kinds: Vec<PathKind>,
}

impl SessionSignalsAccumulator {
    pub fn new(session_id: &str, source: &str) -> Self {
        Self {
            signals: SessionSignals {
    ...
    pub fn push(&mut self, chunk: &ActivityChunk) {
        if let Some(ms) = parse_ts_ms(&chunk.created_at) {
            self.stamps.push(ms);
        }
        match chunk.action_type.as_str() {
            ACTION_TYPE_TASK_FAILED => self.interrupts += 1, // Codex reports aborts explicitly
            ACTION_TYPE_ASSISTANT => self.signals.assistant_turns += 1,
            ACTION_TYPE_TOOL_CALL => {
                self.signals.tool_calls += 1;
                self.chain += 1;
                let kind = classify_tool(&chunk.function);
                if kind == ToolKind::Todo && !self.signals.has_edit {
                    self.planned_first = true;
                }
                if kind == ToolKind::Delegate {
                    self.signals.delegate_calls += 1;
                }
                if kind == ToolKind::Edit {
                    self.signals.edit_calls += 1;
                    self.signals.has_edit = true;
                    self.pending_postedit = true;
                    if let Some(p) = tool_path(&chunk.args) {
                        self.edit_kinds.push(classify_path(p));
                    }
                    ...
                    self.lines_added += num("linesAdded");
                    self.lines_removed += num("linesRemoved");
                }
            }
            ACTION_TYPE_RAW if chunk.function == FUNCTION_USER_MESSAGE => {
                ...
                if self.chain > 0 {
                    self.chains.push(self.chain);
                    self.signals.max_chain = self.signals.max_chain.max(self.chain);
                    self.chain = 0;
                }
                self.signals.user_turns += 1;
                let words = text.split_whitespace().count() as i64;
                self.signals.prompt_words += words;
                self.signals.longest_prompt_words = self.signals.longest_prompt_words.max(words);
                if !self.first_brief_seen {
                    self.first_brief_seen = true;
                    let head: String = text.chars().take(BRIEF_SCAN_CHARS).collect();
                    self.signals.brief_items = list_item_re().find_iter(&head).count() as i64;
                    self.signals.brief_constraints =
                        constraint_re().find_iter(&head).count() as i64;
                    self.signals.brief_targets = target_re().find_iter(&head).count() as i64;
                }
                self.questions +=
                ...
                if self.pending_postedit {
                    self.pending_postedit = false;
                    self.signals.postedit_turns += 1;
                    if text.contains("```") || text.matches('\n').count() >= 3 {
                        self.postedit_paste += 1;
                    }
                    if evidence_re().is_match(text) {
                        self.postedit_evidence += 1;
                    } else if ack_re().is_match(text) || text.chars().count() < SHORT_REPLY_CHARS {
                        self.postedit_moveon += 1;
                    }
                }
            }
            _ => {}
    ...
    pub fn finish(mut self) -> SessionSignals {
        if self.chain > 0 {
            self.chains.push(self.chain);
            self.signals.max_chain = self.signals.max_chain.max(self.chain);
        }

        let users = self.signals.user_turns.max(1) as f64;
        let turns = (self.signals.user_turns + self.signals.assistant_turns).max(1) as f64;
        let edits = self.edit_kinds.len().max(1) as f64;
        let postedit = self.signals.postedit_turns.max(1) as f64;

        self.signals.tools_per_user = self.signals.tool_calls as f64 / users;
        self.signals.mean_chain = if self.chains.is_empty() {
            0.0
        } else {
            self.chains.iter().sum::<i64>() as f64 / self.chains.len() as f64
        };
        self.signals.interrupt_rate = self.interrupts as f64 / users;
        self.signals.user_share = self.signals.user_turns as f64 / turns;
        self.signals.question_rate = self.questions as f64 / users;
        self.signals.redirect_rate = self.redirects as f64 / users;

        let share = |kind: PathKind| {
            self.edit_kinds
                .iter()
        ...
        self.signals.harness_edit_share = share(PathKind::Harness);
        self.signals.infra_edit_share = share(PathKind::Infra);
        self.signals.doc_edit_share = share(PathKind::Doc);
        self.signals.test_edit_share = share(PathKind::Test);
        self.signals.product_edit_share = share(PathKind::Product);

        self.signals.postedit_evidence_rate = self.postedit_evidence as f64 / postedit;
        self.signals.postedit_moveon_rate = self.postedit_moveon as f64 / postedit;
        self.signals.postedit_paste_rate = self.postedit_paste as f64 / postedit;

        self.signals.lines_added = self.lines_added;
        self.signals.lines_removed = self.lines_removed;
        self.signals.planned_first = self.planned_first;

        let spans = active_spans(self.stamps);
        self.signals.longest_span_secs = spans
            .iter()
            .map(|(a, b)| (b - a) as f64 / 1000.0)
            .fold(0.0, f64::max);
        self.signals.started_at_ms = spans.first().map(|(a, _)| *a).unwrap_or(0);
        self.signals.active_secs = spans.iter().map(|(a, b)| (b - a) as f64 / 1000.0).sum();
        self.signals.active_spans = spans;
        self.signals
    }
}

/// Extract one session's signals from an already-materialized chunk slice.
pub fn extract(session_id: &str, source: &str, chunks: &[ActivityChunk]) -> SessionSignals {
    let mut accumulator = SessionSignalsAccumulator::new(session_id, source);
    for chunk in chunks {
...
mod tests {
    ...
    #[test]
    fn incremental_pages_preserve_the_existing_signal_projection() {
        let chunks = vec![
            user("add a parser test", "2026-07-01T10:00:00Z"),
            tool(
                "Read",
            ...
            user("still fails with 404?", "2026-07-01T10:01:00Z"),
        ];
        let expected = extract("s", "claude_code", &chunks);
        let mut accumulator = SessionSignalsAccumulator::new("s", "claude_code");
        for page in chunks.chunks(2) {
            for chunk in page {
                accumulator.push(chunk);
            }
        }
