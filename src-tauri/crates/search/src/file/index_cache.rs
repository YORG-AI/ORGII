                ...
                CacheAction::Build { flight, generation } => {
                    ...
                    let accepted = {
                        ...
                        if owns_flight && slot.generation == generation {
                            let now = Instant::now();
                            if estimated_bytes <= self.max_index_bytes {
...
mod tests {
    ...
    fn test_cache(max_cached_indexes: usize) -> FilePathIndexCache {
    ...
    #[test]
    fn cache_prunes_to_global_byte_budget() {
        let cache = FilePathIndexCache::new(Duration::from_secs(60), 4, usize::MAX, 100);

        for index in 0..3 {
            cache
                .get_or_build(&format!("/repo-{index}"), &[], || {
                    vec![entry(&"x".repeat(40))]
                })
                .unwrap();
        }
    ...
    #[test]
    fn oversized_index_is_shared_but_not_retained() {
        ...
        assert_eq!(build_count.load(Ordering::SeqCst), 1);
        assert!(cache.state.lock().unwrap().slots.is_empty());
    }
}
