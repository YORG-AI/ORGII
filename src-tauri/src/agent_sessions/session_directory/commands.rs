...
/// Per-session context status for the quota monitoring panel.
...
///
/// Shares the same five-minute TTL cache as the Feishu status bar.
#[tauri::command]
...
/// Get the current round count and token total for one session.
#[tauri::command]
