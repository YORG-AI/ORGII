...
mod tests {
    ...
    use crate::orgtrack::types::{
        OrgtrackRawEvent, OrgtrackRawEventSource, OrgtrackSessionTrajectory,
    };
    ...
    fn sqlite_root_descriptor_restores_path_and_ten_mib_omitted_sibling_exactly() {
        ...
        let restored: Value = serde_json::from_str(edit.args_json.as_deref().unwrap())
            .expect("restored edit args JSON");
    ...
    fn same_generation_revision_change_rejects_pinned_trajectory_cursor() {
        ...
        let error =
            validate_pinned_replay_cursor("codexapp-revision", "same-generation", 11, &cursor)
