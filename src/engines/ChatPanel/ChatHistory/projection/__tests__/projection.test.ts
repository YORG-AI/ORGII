...
describe("projection deltas", () => {
  it("reports upserts, removals and canonical order", () => {
    ...
    if (delta.kind !== "replace") throw new Error("expected replace delta");
  ...
  it("uses the compact append protocol for unchanged prefixes", () => {
    const first = event("1", "user");
  ...
  it("sends only appended events for an immutable append", () => {
    const first = event("1", "user");
    const second = event("2");
