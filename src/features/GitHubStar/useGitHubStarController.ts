...
export function useGitHubStarController({
  ...
  const commitResult = useCallback((result: GitHubStarResult): void => {
    if (result.status === "starred") {
      // Confirmation is terminal for the browser-fallback flow. Stop the
      // focus listener so a settings row that stays mounted does not invoke
      // `gh` again on every future application focus.
      fallbackOpenedRef.current = false;
