...
| 3   | Pick a **same-repo** PR, confirm                                         | Confirm shows "Resolving PR..." + spinner while `worktree_resolve_pr_base` fetches; on success pill shows PR label; source gains `resolvedBaseRef` = PR head SHA       |
