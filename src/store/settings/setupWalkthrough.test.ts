...
import { handleExternalChangeAtom, settingsAtom } from "./settingsAtom";
import {
...
describe("setup walkthrough outcome migration", () => {
  ...
  it("applies the same migration to externally reloaded settings", () => {
    const store = createStore();

    store.set(handleExternalChangeAtom, { "general.theme": "github-dark" });

    expect(store.get(settingsAtom)[SETUP_WALKTHROUGH_OUTCOME_KEY]).toBe(
      "completed"
    );
  });
