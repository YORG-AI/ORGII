...
export const handleExternalChangeAtom = atom(
  ...
  (_get, set, rawSettings: Record<string, unknown>) => {
    const validated = validateSettings(rawSettings);
    // Keep the external-watch entry point aligned with startup migration.
    // Otherwise an established profile whose older file lacks the outcome key
    // would inherit the schema default ("open") and be redirected into setup.
    validated[SETUP_WALKTHROUGH_OUTCOME_KEY] =
      resolveSetupWalkthroughOutcome(rawSettings);
    set(settingsAtom, validated);
  }
);
