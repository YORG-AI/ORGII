 ...
 *   panel drives `builder_profile_extract` in bounded batches and re-scores as
 *   coverage grows, instead of blocking on tens of thousands of transcripts.
 *   The progress bar is that backlog draining.
 * - A `?` in the code is a real answer. It means a letter failed one of the two
 *   gates — the sessions disagreed, or the verdict hinged on where the neutral
 *   point was placed — and is shown as "not measurable", never as a midpoint.
 */
export default function BuilderProfilePanel() {
  ...
  // Drain the extraction backlog while the panel is open, re-scoring as it goes.
  useEffect(() => {
    let cancelled = false;
  ...
  return (
    <div className={SECTION_GAP_CLASSES} data-testid="builder-profile-panel">
      ...
      {/* The code. Letters that failed a gate render as a dimmed ?, and say why. */}
      <div className="flex flex-col items-center gap-2 rounded-lg bg-bg-2 px-4 py-6">
        ...
        {!profile.hasEnoughSessions && (
          <div className="text-xs text-warning-5">{t("tooFewSessions")}</div>
        )}
      </div>
