...
const payload: JourneyGraphPayload = { nodes: [{ id: "session/s", kind: "session", evidenceClass: "canonical", sourceRef: "session:s" }], edges: [], coverage: [{ sourceRef: "session:s", status: "represented" }] };
describe("shared journey graph", () => {
  ...
  it("surfaces evidence and fails closed on partial coverage", () => { expect(graphToJourneyViewModel(payload).nodes[0].evidenceClass).toBe("canonical"); expect(() => graphToJourneyViewModel({ ...payload, coverage: [{ sourceRef: "session:s", status: "uncovered" }] } as JourneyGraphPayload)).toThrow("incomplete"); });
});
