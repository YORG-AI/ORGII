...
export const StorylineTimeline: React.FC<{ viewModel: StorylineViewModel }> = ({
  ...
  >
    <p className="text-xs text-text-3">
      Real-time x-axis uses display timestamps only. Compressed spans are
      labeled; connectors are factual edges.
    </p>
    {viewModel.lanes.map((lane) => (
      <div
        key={lane.id}
        className="overflow-x-auto border-l-2 border-border-2 pl-3"
      >
        <h3 className="mb-2 text-sm font-medium text-text-1">
          Agent / session lane: {lane.label}
        </h3>
        <div className="mb-2 min-w-max border-b border-border-2 pb-1 text-[11px] text-text-3">
          Real-time x-axis -&gt;
        </div>
        <ol className="flex min-w-max items-stretch gap-2">
          {lane.milestones.map((milestone) => (
            <React.Fragment key={milestone.id}>
              <li
                className="w-52 shrink-0 border border-border-2 p-2 text-xs"
              ...
              {lane.gaps
                .filter(
                  (gap) => gap.fromTimestamp === milestone.displayTimestamp
                )
                .map((gap) => (
                  <li
                    key={`${gap.fromTimestamp}-${gap.toTimestamp}`}
                    className="flex w-40 shrink-0 items-center border-x border-dashed border-warning-6 px-2 text-xs text-warning-6"
                    data-testid="storyline-idle-gap"
                  >
                    Idle compression: {Math.round(gap.durationMs / 60000)} min
                  </li>
                ))}
            </React.Fragment>
          ))}
