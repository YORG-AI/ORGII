...
export interface BrowserMobileAuthClientOptions {
...
export function createBrowserMobileAuthClient({
  oauthStorage,
  fetcher,
}: BrowserMobileAuthClientOptions): MobileAuthClient {
  ...
  return {
    ...
    async establishServerSession(accessToken) {
      ...
      try {
        response = await fetcher(SERVER_SESSION_PATH, {
          method: "POST",
          credentials: "include",
    ...
    async signOut(session) {
      await fetcher(SERVER_SESSION_PATH, {
        method: "DELETE",
        credentials: "include",
      }).catch(() => undefined);
      if (session) {
